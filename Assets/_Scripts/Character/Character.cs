...
namespace Characters
{
    public abstract class Character
    {
        ...
        public Character(string name, CharacterConfigData config, GameObject prefab)
        {
            ...
            if (prefab != null)
            {
                GameObject ob = Object.Instantiate(prefab, characterManager.characterPannel);
        ...
        public List<string> FormatDialogue(List<string> lines)
        {
            List<string> formatDialogue = new List<string>();
            foreach (string line in lines)
            {
                formatDialogue.Add($"{displayName} \"{line}\"");
            }
            return formatDialogue;
        }

        public virtual Coroutine Show(float speed = 1f)
        {
            if (isRevealing)
            {
                return co_revealing;
            ...
            co_revealing = characterManager.StartCoroutine(ShowingOrHiding(true, speed));

            return co_revealing;
        }

        public virtual Coroutine Hide(float speed = 1f)
        {
            if (isHiding)
            {
                return co_hiding;
            ...
            co_hiding = characterManager.StartCoroutine(ShowingOrHiding(false, speed));
        ...
        {
            Debug.Log("Color changing is no applicable to  this character type!");
            yield return null;
        }
        ...
        public virtual IEnumerator Highlighting(bool hightlight, float speedMultiplier, bool immediate = false)
        {
